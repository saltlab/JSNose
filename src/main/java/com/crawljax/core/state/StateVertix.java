...
public class StateVertix implements Serializable {
	...
	public boolean searchForCandidateElements(CandidateElementExtractor candidateExtractor,
			...
			boolean clickOnce, StateFlowGraph sfg, boolean isEfficientCrawling, boolean isRandomEventExec) {
		...
		try {
			...
			//Amin: randomizeCandidateElement is by default false (clickable order)
			if (isRandomEventExec)
				Collections.shuffle(candidateList);

			// Amin: Sorting CandidateElement w.r.t event productivity
			// TODO: The code should be re-written later. 
			// there is technical problem with in-place sorting and using Collections.sort
			if (isEfficientCrawling){
				int[] indices = new int[candidateList.size()];
				double[] productivity = new double[candidateList.size()];
				
				for (int i=0; i<candidateList.size(); i++){
					indices[i]=i;
					productivity[i]= sfg.getEventProductivity(candidateList.get(i));
				}
				
				int temp_idx; double temp_productivity;
				for (int i=0; i<candidateList.size(); i++)
					for (int j=i; j<candidateList.size(); j++)
						if (productivity[i] < productivity[j]){
							temp_idx = indices[i];  indices[i] = indices[j]; indices[j] = temp_idx;
							temp_productivity = productivity[i];  
							productivity[i] = productivity[j]; 
							productivity[j] = temp_productivity;
						}

				//LOGGER.info("Sorting events based on event productivity ratio");
				for (int i=0; i<candidateList.size(); i++)
				{
					for (String eventType : eventTypes) { 
						if (eventType.equals(EventType.click.toString())) {
							candidateActions.add(new CandidateCrawlAction(candidateList.get(indices[i]),
									EventType.click));
							//LOGGER.info("Added " + candidateList.get(indices[i]).getGeneralString() + " with PR:" + productivity[i]);
						} else {
